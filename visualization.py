    ...
    def draw_grid(self,positions = None): 
        ...
        if positions:
            for position in positions:
                ...
                pygame.draw.rect(self.screen, self.GREEN, (*top_left, self.tile_size, self.tile_size))

        # Draw Grid Lines
        for row in range(self.height_cell_count): 
            pygame.draw.line(self.screen, self.BLACK, (0, row * self.tile_size), (self.screen_width, row * self.tile_size))

        for col in range(self.width_cell_count):
            pygame.draw.line(self.screen, self.BLACK, (col * self.tile_size, 0), (col * self.tile_size, self.screen_height))
