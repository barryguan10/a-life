...
def gen(num):
    ...
    return {
        (randrange(0, WIDTH_CELL_COUNT), randrange(0, HEIGHT_CELL_COUNT))
        for _ in range(num)
    }
...
def draw_grid(positions):
    ...
    for position in positions:
        col, row = (
            position  # It's "Col, Row" due to X, Y (think 2D-Cartesian Coordinates)
        )
    ...
    for row in range(HEIGHT_CELL_COUNT):
        pygame.draw.line(
            screen, BLACK, (0, row * TILE_SIZE), (SCREEN_WIDTH, row * TILE_SIZE)
        )

    for col in range(WIDTH_CELL_COUNT):
        pygame.draw.line(
            screen, BLACK, (col * TILE_SIZE, 0), (col * TILE_SIZE, SCREEN_HEIGHT)
        )
